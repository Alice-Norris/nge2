...
def read_file(filename: str):
  ...
  with open(path, mode='rb') as file:
    ...
    book_name = file.read(8).decode(utf8).strip()

    # Creating new book
    book = Book(book_name)
    
    # Reading number of sheets from files
    num_sheets = int.from_bytes(file.read(1), "big")
    ...
    for x in range(num_sheets):
      ...
      sheet_name = file.read(8).decode(utf8).strip()
      sheet_names.append(sheet_name)
    
    # Checking and clearing end of header byte
    if file.read(1) != b'\x19':
    ...
    for sheet in book.sheets:
      ...
      num_chars = int.from_bytes(file.read(1), "big")
      ...
      for x in range(num_chars):
        ...
        char_name = file.read(8).decode(utf8).strip()
...
def write_file(book: Book=None):
  ...
  # number of sheets
  num_sheets_bytes = bytes([len(active_book.sheets)])

  # name of file
  book_name = active_book.name
  ...
  ### Writing to file
  with open( active_book.name + '.nge', mode='wb') as file:
